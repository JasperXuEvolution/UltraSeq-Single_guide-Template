"""
README - UltraSeq Bootstrapping Analysis Script
-------------------------------------------------
Description:
    This script performs deep sequencing analysis for tumor data using a bootstrapping
    approach to generate summary statistics. The analysis compares experimental mouse
    groups and control groups (either Cas9-Negative mice or plasmid controls). Bootstrapping
    is used to account for variability in the data, and the script calculates several metrics,
    including tumor burden (TTB), tumor number (TTN), LN_mean, geometric mean, and various
    percentile-based metrics. Additionally, it computes relative normalized metrics by comparing
    experimental data with control data.

Features:
    • Supports multiple bootstrapping replicates.
    • Two modes of operation:
         - Normal mode (using Cas9-Negative mice as control)
         - Plasmid mode (using plasmid data as control)
    • Option for an adaptive method (combined effect) to match tumor numbers.
    • Calculation of both gRNA-level and gene-level summary statistics.
    • Ability to exclude specific sgRNAs or samples from the analysis.
    • Outputs intermediate bootstrapping results and final summary CSV files.
...
Example 1: Normal method with Cas9-Negative mice as control (non-adaptive)
    python3 UltraSeq_Boostrapping_GW.py \
        --a0 /path/to/Met_testing_data-Tumor.parquet \
        --a2 300 --a3 300 --a4 10 --a5 KTC --a6 KT \
        --o1 /path/to/Output/Test_BT \
        --o2 /path/to/Output/Test_BT \
        --l1 50 60 70 80 90 95 97 99 \
        --m N --c No

Example 2: Normal method with Adaptive method
    python3 UltraSeq_Boostrapping_GW.py \
        --a0 /path/to/Met_testing_data-Tumor.parquet \
        --a2 300 --a3 300 --a4 10 --a5 KTC --a6 KT --a7 300 \
        --o1 /path/to/Output/Test_BT \
        --o2 /path/to/Output/Test_BT \
        --l1 50 60 70 80 90 95 97 99 \
        --m N --c Yes

Example 3: Plasmid mode with Normal method
    python3 UltraSeq_Boostrapping_GW.py \
        --a0 /path/to/Met_testing_data-Tumor.parquet \
        --p /path/to/Met_testing_data-Plasmid.parquet \
        --a2 300 --a4 10 --a5 KTC \
        --o1 /path/to/Output/Test_BT \
        --o2 /path/to/Output/Test_BT \
        --l1 50 60 70 80 90 95 97 99 \
...
Example 4: Plasmid mode with Adaptive method
    python3 UltraSeq_Boostrapping_GW.py \
        --a0 /path/to/Met_testing_data-Tumor.parquet \
        --p /path/to/Met_testing_data-Plasmid.parquet \
        --a2 300 --a4 10 --a5 KTC --a7 100 \
        --o1 /path/to/Output/Test_BT \
        --o2 /path/to/Output/Test_BT \
        --l1 50 60 70 80 90 95 97 99 \
        --m P --c Yes
...
Arguments:
    --a0 : Address of processed tumor data (Parquet format).
    --a1 : (Optional) Address of sample exclusion list.
    --a2 : Cell number cutoff for focal genotype.
    ...
    --l1 : (Optional) List of tumor size quantiles to calculate (e.g., 50 60 70 80 90 95 97 99).
    --l2 : (Optional) List of sgRNA sequences to exclude.
...
Output:
    Depending on the options and bootstrapping replicates, the script produces:
        • gRNA-level summary CSV files.
        • Gene-level summary CSV files.
        • (Optional) Intermediate output CSV files containing bootstrapped metrics.
...
def Generate_AC_data(df1, minimal_tumor_size, gRNA_dic):
    """
    Optimized function to generate a new DataFrame with matched tumor numbers for each gRNA.

    Parameters:
    - df1 (pd.DataFrame): Input DataFrame containing tumor data.
    - minimal_tumor_size (int): The size of minimal tumors to create when data is insufficient.
    - gRNA_dic (dict): Dictionary with gRNA keys and tumor count values.

    Returns:
    - pd.DataFrame: A new DataFrame with matched tumor numbers for each gRNA.
    """
    # Step 1: Pre-sort the DataFrame for faster access
    df1 = df1.sort_values(by=['gRNA', 'Cell_number'], ascending=[True, False])

    # Step 2: Group the DataFrame by gRNA for fast lookup
    grouped = df1.groupby('gRNA')

    # Step 3: Precompute Sample_ID distributions for fake tumors
    sample_distributions = {
        gRNA: group['Sample_ID'].value_counts(normalize=True)
        for gRNA, group in grouped
    }

    # Step 4: Prepare the final list of DataFrames
    df2_list = []

    # Step 5: Process each gRNA
    for gRNA, required_count in gRNA_dic.items():
        if gRNA in grouped.groups:
            # Get the sorted group for this gRNA
            gRNA_df = grouped.get_group(gRNA)

            # Determine how many rows are available
            num_available = len(gRNA_df)

            if num_available >= required_count:
                # If enough rows are available, take the top rows
                top_tumors = gRNA_df.iloc[:required_count]
            else:
                # If not enough rows, take all available rows and create fake tumors
                top_tumors = gRNA_df.copy()
                num_missing = required_count - num_available

                # Create fake tumors using vectorized operations
                fake_tumors = pd.DataFrame([gRNA_df.iloc[0]] * num_missing) if not gRNA_df.empty else pd.DataFrame()
                if not gRNA_df.empty:
                    fake_tumors['Cell_number'] = minimal_tumor_size

                    # Assign Sample_IDs based on precomputed distributions
                    sample_dist = sample_distributions[gRNA]
                    fake_tumors['Sample_ID'] = np.random.choice(
                        sample_dist.index, size=num_missing, p=sample_dist.values
                    )

                    # Combine real and fake tumors
                    top_tumors = pd.concat([top_tumors, fake_tumors], ignore_index=True)

        else:
            print(f"Warning: {gRNA} is missing in the tumor data")
            top_tumors = pd.DataFrame()

        # Add to the result list
        df2_list.append(top_tumors)

    # Step 6: Concatenate all subsets to form the final DataFrame
...
def Calculate_Relative_Normalized_Metrics(input_df1, input_df2, percentile_list, group_trait='gRNA'):
    ...
    Parameters:
        input_df1 (pd.DataFrame): Experimental data.
        input_df2 (pd.DataFrame or None): Control data.
        percentile_list (list): Percentiles to calculate.
        group_trait (str): Grouping trait, default is 'gRNA'.

    Returns:
        pd.DataFrame: Metrics with normalization, relative values, and annotations.
    """
    # Group data
    ...
    if input_df2 is not None:
        # Calculate metrics for the control dataset
        temp_df2 = (
            input_df2.groupby([group_trait, 'Type'], as_index=False)
            .agg(
                TTB=('Cell_number', 'sum'),
                TTN=('Cell_number', 'count')
            )
        )
        temp_out = Generate_Normalized_Metrics(temp_df, temp_df2, ['TTN', 'TTB'], group_trait)
        temp_df = temp_df.merge(temp_out, on=[group_trait])

    # Add relative metrics
    Add_Corhort_Specific_Relative_Metrics(temp_df, group_trait)

    # Annotate sample type if group_trait is 'gRNA'
    if group_trait == 'gRNA':
        temp_df = temp_df.merge(
            input_df1[['gRNA', 'Targeted_gene_name', 'Numbered_gene_name']].drop_duplicates(),
            how='inner', on='gRNA'
        )
...
def hill_estimator(cell_numbers, percentile=95):
    """
    Compute Hill's estimator for extreme value analysis.

    Parameters:
    - cell_numbers (array-like): A list or numpy array of tumor sizes.
    - percentile (float): The percentile threshold to consider (default is 95).
    ...
    # Select tumors above the threshold
    above_threshold = cell_numbers[cell_numbers > threshold]

    if len(above_threshold) == 0:
        return np.nan  # Return NaN if no values exceed the threshold
